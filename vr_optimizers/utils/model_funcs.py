...
def get_vr_method(method_name, model, train_set, batch_size, lr, device, num_workers, weight_decay, data_noise, label_noise):
    ...
    elif method_name.startswith('sarah') or method_name.startswith('q-sarah') or method_name.startswith('e-sarah') or method_name.startswith('spider'):
        vr_method_class = optims.SARAH
    else:
        raise ValueError(f'{method_name} is not valid VR method.')

    if method_name.startswith('spider'):
        order = 0
        clip = 0
        para_str = method_name.split('_')[0][6:].replace('d','.')
        order = int(para_str[0])
        if order==2 and '=' in para_str:
            clip = para_str[1:].split('=')
            clip = [float(i) for i in clip]
        else:
            clip = float(para_str[1:])
        print("======spider with the following parameters=====")
        print(para_str, order, clip)
        method_name = 'sarah_' + method_name.split('_')[1]
        print("+++sarah method name:", method_name)
        return vr_method_class(method_name, model, train_set, batch_size, lr, device, num_workers, weight_decay, order, clip, noise = (data_noise, label_noise))

    return vr_method_class(method_name, model, train_set, batch_size, lr, device, num_workers, weight_decay, noise = (data_noise, label_noise))
