...
def plot(exps, log_scale=True, legend=None, file=None,
    ...
    for i, exp in enumerate(exps):
        ...
        plot_mean_std(ax, runs, metric_name, i, label=legend[i])
...
def plot_mean_std(ax, runs, metric_name, i, label):
    ...
    print(label, '  accuracy is last ({}) best ({})'.format(mean[-1],max(mean)))

    x = np.arange(0, len(mean))
    ax.plot(x, mean, marker=MARKERS[i], markersize=12, markevery=len(x) // 10, label=label)
