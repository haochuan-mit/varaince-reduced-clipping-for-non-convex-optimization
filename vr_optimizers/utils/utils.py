...
def add_noise(data, label, noise_level=(1.0,0.1)):
    noise = torch.randn_like(data)*(noise_level[0]/data.shape[-1])
    data_res = data + noise
    replace = np.random.rand(len(label)) < noise_level[1]
    label_res = copy.deepcopy(label)
    for i in range(len(label_res)):
        if replace[i]:
            label_res[i] = np.random.randint(10)
