...
class SARAH(VROptimizer):
    def __init__(self, name, model, train_set, batch_size, lr, device, num_workers, weight_decay, order=0, clip=0, noise=None):
        ...
        if self.method_name == 'sarah':
            ...
            if self.method_name == 'q-sarah':
                self.big_bs = int(np.min([np.ceil(self.j**2), self.n]))
            else:  # E-SARAH
                self.big_bs = int(np.min([1.1**self.j, self.n]))
        self.small_bs = int(np.ceil(self.big_bs**(1/2)))
    ...
    def run_one_iter(self):
        ...
        # adjust grad
        set_vr_grad(self.model, self.model_snap, self.big_grad, clip_order=self.order, clip=self.clip)
        # save current model to snap
        self.model_snap.load_state_dict(deepcopy(self.model.state_dict()))
        # adjust big grad estimator
        self.big_grad = [p.grad for p in self.model.parameters()]
        # do grad step
        self.optimiser.step()
